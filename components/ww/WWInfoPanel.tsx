...
export const WWInfoPanel: React.FC<InfoPanelProps> = ({ activeGuide, showAnimation, setShowAnimation, accentColor, faction, description, isMobile }) => {
  return (
        ...
        >
            ...
            {activeGuide?.content?.spine && !isMobile && (
                <button title="Action"
                    ...
                    className="bg-black/80 md:bg-black/60 hover:bg-black/80 border border-white/20 text-white/90 text-sm px-4 py-2 rounded-full md:backdrop-blur-md transition-all self-start flex items-center gap-2 mb-6 pointer-events-auto"
                >
            ...
            <div className="flex flex-col gap-3 max-w-[320px] mb-8">
                ...
                <Link href="/tier-list" className="group relative w-full flex items-center justify-between px-6 py-4 rounded-xl text-gray-800 bg-white md:bg-white/95 md:backdrop-blur-xl border border-white text-sm font-bold tracking-wider transition-all duration-300 hover:scale-[1.02] outline-none shadow-[0_4px_20px_rgb(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)]">
